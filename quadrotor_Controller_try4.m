...
function [u, e] = quadrotor_Controller_try4(controller_input)
...
for i = 1:size(Direction, 2)
    Rd(:, i) = cross(relative_P(:, i), Direction(:, i));
end
...
% gamma_position = controller_input.K_position * error_position;
...
w = 1;
% ControlMatrix = Rd'*Rd + w*eye(m);
% u = pinv(ControlMatrix) * (gamma_orientation'*Rd)';

C = [Rd; Direction(3, :)];
ControlMatrix = C'*C + w*eye(m);
RHS = [(gamma_orientation'*Rd)'; sum(controller_input.robot.nodes_masses) * (controller_input.robot.g + controller_input.acc)];

u = pinv(ControlMatrix) * RHS;
